...
class QzSecurityController extends Controller
{
    ...
    {
        // DomPDF available? Standard renderer path.
        ...
        // No DomPDF? Serve a REAL PDF built in-house. The previous HTML
        // fallback broke QZ printing: QZ Tray downloads this URL ITSELF
        // (outside the browser — no cookies/session) and a ~755-byte HTML
        // page is not a PDF, so every silent PDF print failed with
        // "Cannot parse (FILE)… as a PDF file: End-of-File, expected line".
        return response($this->buildTestPdf(), 200, [
            'Content-Type'        => 'application/pdf',
            'Content-Disposition' => 'inline; filename="qz-test.pdf"',
            'Cache-Control'       => 'no-store',
        ]);
    }

    /**
     * Minimal single-page test PDF assembled byte-by-byte (no dependencies).
     * Byte offsets for the xref table are computed while writing, so the
     * result is a strictly valid PDF 1.4 file that PDFBox (QZ Tray),
     * Acrobat and browsers all accept.
     */
    private function buildTestPdf(): string
    {
        ...
        $stream = "BT /F1 16 Tf 56 780 Td (Laravel QZ Tray - Test Document) Tj ET\n"
            . "BT /F1 10 Tf 56 756 Td 14 TL\n"
            . '(' . $esc('Generated: ' . now()->toDateTimeString()) . ") Tj T*\n"
            . '(' . $esc('Host: ' . (request()->getHttpHost() ?: 'localhost')) . ") Tj T*\n"
            . "(Built-in minimal PDF writer - no DomPDF required on the server.) Tj T*\n"
            . "(If this page prints silently, QZ Tray PDF printing works.) Tj T*\n"
            . "ET\n";
